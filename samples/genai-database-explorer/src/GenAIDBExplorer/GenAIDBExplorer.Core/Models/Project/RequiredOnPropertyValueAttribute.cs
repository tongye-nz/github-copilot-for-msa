...
internal sealed class RequiredOnPropertyValueAttribute : ValidationAttribute
{
    ...
    protected override ValidationResult? IsValid(object? value, ValidationContext validationContext)
    {
        ...
        if (Equals(OtherPropertyValue, otherPropertyValue))
        {
            ...
            else if (NotEmptyOrWhitespace && string.IsNullOrWhiteSpace(value.ToString()))
            {
                return new ValidationResult($"Property '{validationContext.DisplayName}' cannot be empty or whitespace when '{OtherPropertyName}' is {OtherPropertyValue}.");
            }
            else
            {
